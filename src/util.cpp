...
#else
    const char* pszModule = "meconcash";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Meconcash
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Meconcash
    // Mac: ~/Library/Application Support/Meconcash
    // Unix: ~/.meconcash
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Meconcash";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "meconcash.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "meconcashd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Meconcash will not work properly.");
...
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments)
{
    ...
    ss << "/";
    ss << "Meconcash:" << FormatVersion(MECONCASH_VERSION);
    ss << "(" << CLIENT_BUILD << ")/";
    return ss.str();
}
