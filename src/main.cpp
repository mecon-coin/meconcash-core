...
const string strMessageMagic = "Meconcash Signed Message:\n";
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        if (IsProofOfWork() && nHeight != 0 && pindexPrev->nMoneySupply > LAST_POW_MONEY)
...
bool LoadBlockIndex()
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "Meconcash", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
...
bool InitBlockIndex() {
    ...
    fTxIndex = true; // meconcash: txindex is always enabled
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("MeconcashMiner : proof-of-work not meeting target");

    //// debug print
    printf("MeconcashMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MeconcashMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MeconcashMiner : ProcessBlock, block not accepted");
    }
...
#endif
{
    ...
    try { ploop {
        ...
        printf("Running MeconcashMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        if(!fProofOfStake) printf("MeconcashMiner terminated\n");
        throw;
    }
}
